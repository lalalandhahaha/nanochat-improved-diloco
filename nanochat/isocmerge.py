"""ISOCMerge: Isotropic Covariance (ISO-C) merging-based gradient aggregation optimizer.

Copied from isoloco-main/src/tplr/isocloco.py (class ISOCLoCo), adapted for nanochat:
- Renamed to ISOCMerge.
- SVD is computed in fp32 and cast back (torch.linalg.svd does not support bf16),
  since nanochat keeps some params (wte, value_embeds) in bf16.

Used as the DiLoCo outer optimizer when --diloco-outer-opt=isoc. The pseudo-gradients
(w_old - w_new) are placed in p.grad by the DiLoCoWrapper; this optimizer then gathers
them across workers, merges them with ISO-C, and applies the update itself.
"""
...
class ISOCMerge(torch.optim.Optimizer):
    """
    ISOCMerge optimizer that aggregates gradients using ISO-C (Isotropic Covariance) merging.
    ...
    def __init__(
        ...
        """
        Initialize ISOCMerge optimizer.
    ...
    def _apply_isoc(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            Orthogonalized tensor (same dtype as input)
        """
        # torch.linalg.svd does not support bf16/fp16: compute in fp32, cast back
        orig_dtype = tensor.dtype
        t = tensor.float() if orig_dtype not in (torch.float32, torch.float64) else tensor
        U, S, Vh = torch.linalg.svd(t, full_matrices=False)
        S_mean = torch.ones_like(S) * S.mean()
        merged = torch.linalg.multi_dot((U, torch.diag(S_mean), Vh))
