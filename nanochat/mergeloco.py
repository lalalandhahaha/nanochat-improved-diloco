"""MergeLoCo: Model merging-based gradient aggregation optimizer (TIES-style).

Copied from isoloco-main/src/tplr/mergeloco.py, unchanged in algorithm.

Used as the DiLoCo outer optimizer when --diloco-outer-opt=ties. The pseudo-gradients
(w_old - w_new) are placed in p.grad by the DiLoCoWrapper; this optimizer then gathers
them across workers, merges them with TIES-style sign election, and applies the update.
"""
...
class MergeLoCo(torch.optim.Optimizer):
    ...
    def _merge_gradients(self, gradients_list: List[torch.Tensor]) -> torch.Tensor:
        ...
        if self.ties_disjoint:
            ...
            merged = masked.sum(dim=0) / matching_count.to(stacked.dtype)
        else:
