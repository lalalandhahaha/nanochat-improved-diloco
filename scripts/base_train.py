...
parser.add_argument("--eval-svd", type=int, default=-1, help="evaluate val bpb every N steps (-1 = disable)")
...
parser.add_argument("--use-diloco", type=int, default=1, help="0 no use DiLoCo, 1 use DiLoCo")
parser.add_argument("--diloco-H", type=int, default=100, help="number of inner optimization steps before outer synchronization")
...
if args.use_diloco:
    assert COMPUTE_DTYPE != torch.float16, "DiLoCo is not supported with fp16 (GradScaler requires a torch.optim.Optimizer)"
...
while True:
    ...
    if args.eval_svd > 0 and (last_step or step % args.eval_svd == 0):
